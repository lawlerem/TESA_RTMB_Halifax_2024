...
nll<-function(par){
  ...
  logssb <- log(colSums(exp(logN)*stockMeanWeight*propMature))
  ADREPORT(logssb)
  REPORT(F)
  return(ans)
}
...
plot(yr, exp(pl$logssb), type="l", lwd=5, col="orange", ylim=c(0,550000), xlab="Year", ylab="SSB")
lines(yr, exp(pl$logssb-2*plsd$logssb), lwd=5, lty="dotted", col="orange")
lines(yr, exp(pl$logssb+2*plsd$logssb), lwd=5, lty="dotted", col="orange")

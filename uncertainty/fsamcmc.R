...
nll<-function(par){
  ...
  ADREPORT(logssb)
  lastssb<-exp(logssb)[length(logssb)]
  REPORT(lastssb)  
  return(ans)
}

obj <- RTMB::MakeADFun(nll, par, map=list(logFA=factor(c(1:4,NA,NA,NA))), silent=TRUE)

opt <- nlminb(obj$par, obj$fn, obj$gr, control=list(iter.max=1000,eval.max=1000))
sdrep <- RTMB::sdreport(obj)
pl <- as.list(sdrep, what="Est", report=TRUE)
plsd <- as.list(sdrep, what="Std", report=TRUE)
sdr<-summary(sdrep)
...
fitmcmc2 <- tmbstan(obj, chains=1,
              ...
              lower=rep(-50,length(opt$par)), upper=rep(50,length(opt$par)) )
...
lastssb<-apply(mc[,1,],1, function(x)obj$report(x)$lastssb)
plot(density(lastssb), col="red", lwd=3)
f<-function(x)dlnorm(x, meanlog=tail(pl$logssb,1), sdlog=tail(plsd$logssb,1))
curve(f,10000, 100000,  add=TRUE, col="gray", lwd=3)
